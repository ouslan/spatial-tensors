...
source "/home/osborn/Documents/Github/spatial-tensors/.devenv/profiles/dev/shell-env.sh"
...
export ZDOTDIR="/home/osborn/Documents/Github/spatial-tensors/.devenv/profiles/dev/zsh"
...
if [ ! -x "/usr/bin/zsh" ] && ! command -v "/usr/bin/zsh" >/dev/null 2>&1; then
    echo "devenv: error: shell '/usr/bin/zsh' not found" >&2
    echo "devenv: add zsh to your devenv.nix packages or set SHELL to an absolute path" >&2
    exit 1
fi
exec "/usr/bin/zsh" -i
echo "devenv: error: failed to exec into /usr/bin/zsh" >&2
exit 1
